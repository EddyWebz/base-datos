...
const { v4: uuidv4 } = require('uuid');  // Usamos el paquete 'uuid' para generar IDs únicos
const connection = require('./database/database-promise');

// Configurar multer para manejar la carga de archivos
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, path.join(__dirname, 'BASE-2/uploads'));  // Directorio de subida
    },
...
router.post('/register', upload.array('image', 4), (req, res) => {
    ...
    const images = req.files.map(file => {
        const imagePath = `/uploads/${file.filename}`;  // La ruta de la imagen con el nuevo nombre
        console.log('Ruta de la imagen guardada:', imagePath);  // Verificar la ruta (depuración)
        return imagePath;
    });
...
router.get('/search', async (req, res) => {
    ...
    try {
        ...
        const vehicles = results.map(vehicle => {
            ...
            let images = [];
            try {
                if (typeof vehicle.images === 'string' && vehicle.images.starts.startsWith('[')) {
                    images = JSON.parse(vehicle.images);  // Parsear JSON de imágenes
                } else if (vehicle.images) {
                    images = [vehicle.images];  // Tratar como una cadena simple si no es JSON
                }
